...
import { clearFile } from '@/lib/tools/files';
...
import { ERROR_CODE, dispatchErrorEvent } from '@/lib/moodle/state/errors';
...
export async function createGCRJob(containerName) {
  ...
  try {
    ...
    if (!initResult?.success) {
      ...
      cancelOpenSchedule(room.containerName);
  ...
  try {
    ...
    if (saveData) {
      ...
      if (quizData && quizData.courseid) {
        console.log(await updateCourseVisibility(quizData.courseid, true));
      }
    }
    ...
    cancelSchedule(room.containerName);
    return true;
  } catch (error) {
    console.error(
