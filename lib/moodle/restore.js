...
export async function restoreUsers(
  fileName,
  { baseUrl = process.env.MOODLE_URL, token = process.env.MOODLE_CLOUDSUPPORT_TOKEN } = {}
) {
  try {
    if (!fileName) {
      throw new Error('Missing courseID or fileName');
    ...
    if (!response.ok) {
      ...
      try {
        const errorData = JSON.parse(text);
        return { success: false, error: 'Moodle restore API failed', details: errorData };
      } catch {
        return { success: false, error: 'Moodle restore API failed', details: text };
      }
    }
    ...
    if (result.exception) {
      return { success: false, error: result.message || 'Restore failed', details: result };
    }

    return { success: true, data: result };
  } catch (error) {
    console.error('Restore course error:', error);
    return { success: false, error: 'Restore request failed', details: error.message };
  }
}
...
export async function uploadFile(
  fileName,
  { saveFolder = 'stemp',baseUrl = process.env.MOODLE_URL, token = process.env.MOODLE_CLOUDSUPPORT_TOKEN } = {}
) {
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        return { success: false, error: 'Moodle API error', details: errorData };
      } catch (e) {
        return { success: false, error: 'Moodle API error', details: errorText };
      }
    }
    ...
    return { success: false, error: 'Failed to upload file', details: error.message };
  }
}
