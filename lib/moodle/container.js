import { backupCourse, exportUsers } from '@/lib/moodle/services/backup';
import { ExamRoomModel, FileInfoModel } from '@/lib/db/models';
import { restoreCourse, restoreUsers } from '@/lib/moodle/services/restore';
import { getQuizById } from './db/control';
import { connectDB } from '@/lib/db/connect';
import { STATUS_CODE, dispatchStatusEvent } from './state/status';
import { listenEventRestoreFinished } from './services/webhooks';
import { updateCourseVisibility } from './services/course';
import { uploadFile, clearFile } from '../tools/files';
...
export async function initContainer(containerName) {
  ...
  // hide course on main moodle server
  console.log(await updateCourseVisibility(courseId, false));
...
export async function exportContainer(containerName) {
  ...
  // show course in main moodle server
  console.log(await updateCourseVisibility(targetCourseId, true));
