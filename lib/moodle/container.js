...
export async function initContainer(containerName) {
  ...
  await getTokenContainer(room.containerName);
  ...
  console.log('Import Status: ', restoreUsersStatus);
...
export async function exportContainer(conatinerName) {
  ...
  const restoreCourseStatus = await restoreCourse(targetCourseId, fileCourse.fileName);
